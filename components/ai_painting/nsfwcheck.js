...
class NsfwCheck {
    ...
    async check(base64) {
        ...
        if (this.appid == "" || this.apikey == '' || this.secretkey == "") {
            return {
                message: "百度图片审核服务调用失败，请先配置百度图片审核接口，配置教程：https://github.com/AiPreface/ap-plugin-website/blob/main/docs/Config/docs1.md2 \n\n您也可以发送“#ap全局设置审核关闭”以关闭图片审核功能，关闭后绘制的图片将直接发送，不再进行审核"
            }
        }
        // 鉴黄
    ...
    */
    async bdjh(appid, apikey, secretkey, pic, type = 0) {
        ...
        if (data.error_code == 14) {
            return { message: "百度鉴黄服务调用失败，请先配置百度鉴黄接口，配置教程：https://github.com/AiPreface/ap-plugin-website/blob/main/docs/Config/docs1.md2 \n\n您也可以发送“ap全局设置鉴黄关闭”以关闭鉴黄功能" }
